...
/**
 * http://blog.csdn.net/u014686875/article/details/51124969
 * 
 * @author dq
 *
 */
public class AnimatedGifMaker {
	...
	/**
	 * MAKES ABSOLUTE WHITE (0,0,0) THE TRANSPARENT COLOR.
	 * 
	 * @param c
	...
	public boolean addFrame(Bitmap im) {
		...
		try {
			...
			writeGraphicCtrlExt(); // write graphic control extension THIS IS IT
	...
	protected void getImagePixels() {
		...
		byte[] inputBytes = buffer.array(); // Get the underlying array
											// containing the data.

		pixels = new byte[(3 * inputBytes.length) / 4];
		for (int i = 0; i < inputBytes.length / 4; i++) {
			byte r = inputBytes[4 * i];
			byte g = inputBytes[4 * i + 1];
			byte b = inputBytes[4 * i + 2];
			// ignore alpha
			pixels[3 * i] = r;
			pixels[3 * i + 1] = g;
			pixels[3 * i + 2] = b;
